...
from DIRAC.Core.Utilities.Time import fromString, toEpoch, timeThis, timeBlock
...
def _cacheGetSEsForSites( siteName ):
  if not( siteName in seCache and seCache[siteName]['expire'] > datetime.datetime.now()):
    print "New Entry", siteName
    result = getSEsForSite( siteName )
    seCache[siteName]['data'] = result
    seCache[siteName]['expire'] = datetime.datetime.now() + datetime.timedelta(seconds=300)
  else:
    print  "CacheHit:", siteName
  return seCache[siteName]['data']
...
class JobScheduling( OptimizerExecutor ):
  ...
  def __updateSharedSESites( self, jobState, jobManifest, stageSite, stagedLFNs, opData ):
    ...
    for siteName in siteCandidates:
      ...
      result = _cacheGetSEsForSites( siteName )
      if not result[ 'OK' ]:
        continue
