...
class FileManager( FileManagerBase ):
  ...
  def _getRepIDsForReplica(self,replicaTuples,connection=False):
    ...
    req = "SELECT RepID,FileID,SEID FROM FC_Replicas WHERE (FileID,SEID) IN (%s) AND FileID in (%s)" % (intListToString(queryTuples), intListToString(fileIDs))
    gLogger.info("New Query: %s" % req[:100])
